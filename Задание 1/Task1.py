...
class GLWidget(QtOpenGL.QGLWidget):
    ...
    def Solve(self, Tname):
        ...
        T0 = fsolve(self.Stationary_solution, np.zeros(self.count), args=(0.0))

        self.T = odeint(self.ODE, T0, self.t)
    ...
    def initGeometry(self, Tmax, Tmin, t_i):
        if Tmax < Tmin:
            c = Tmax
            Tmax = Tmin
            Tmin = c
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def initGUI(self):
        ...
        self.Tmax = QtWidgets.QLineEdit('256')
        ...
        self.Tmin = QtWidgets.QLineEdit('-256')
        ...
        self.Time_param = QtWidgets.QLineEdit('12000')
        ...
        gui_layout.addWidget(self.Time_, 11, 0)
    
    # def Time_update(self, t_i):
    #     self.Time_.setText(str(t_i))

    def Plot(self):
        self.glWidget.Plot_sol()

    def Start(self):
        self.glWidget.Time(int(self.Time_param.text()))

        self.glWidget.Parsing(self.Mname.text(), self.Pname.text(), self.Tname.text(), int(self.Tmax.text()), int(self.Tmin.text()))

        for t_i in range(len(self.glWidget.t)):
            self.Time_.setText(str(t_i))
            
            self.glWidget.initGeometry(int(self.Tmax.text()), int(self.Tmin.text()), t_i)
            time.sleep(0.05)
