...
class GLWidget(QtOpenGL.QGLWidget):
    ...
    def Parsing(self, Mname, Pname, Tname, time):
        self.count = 5
        ...
        self.T = self.Solve(Tname, time)

        self.initGeometry()

    def Solve(self, Tname, time):
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def initGUI(self):
        ...
        gui_layout.addWidget(self.Tbtn, 6, 0)

        self.Tname = QtWidgets.QLabel('E:/Python/Задание1/temperature.csv')
        gui_layout.addWidget(self.Tname, 6, 1)

        self.Time_name = QtWidgets.QLabel('Time = ')
        gui_layout.addWidget(self.Time_name, 7, 0)

        self.Time_param = QtWidgets.QLineEdit('20')
        gui_layout.addWidget(self.Time_param, 7, 1)

        self.Start_btn = QtWidgets.QPushButton("Solution start")
        self.Start_btn.clicked.connect(self.Start)
        gui_layout.addWidget(self.Start_btn, 8, 0)

        self.Start_btn = QtWidgets.QPushButton("Plot")
        self.Start_btn.clicked.connect(self.Plot)
        gui_layout.addWidget(self.Start_btn, 8, 1)
        
    def Plot(self):
        self.glWidget.Plot_sol()

    def Start(self):
        self.glWidget.Parsing(self.Mname.text(), self.Pname.text(), self.Tname.text(), int(self.Time_param.text()))
