...
#------------------------------------------------------------------
    ...
    def Verlet_Cyth(self):
        ...
        for t_i in range(len(self.time)):
            ...
            pos_x, pos_y, a_x, a_y = Cyth.Pos(pos_x, pos_y, v_x, v_y, a_x, a_y, mas, self.delt_t, self.num_elem)

            v_x, v_y, a_x, a_y = Cyth.Speed(pos_x, pos_y, v_x, v_y, a_x, a_y, mas, self.delt_t, self.num_elem)
...
#------------------------------------------------------------------
    ...
    def Verlet_CL(self):
        ...
        num_elem_buf = cl.Buffer(cntxt, cl.mem_flags.READ_ONLY | cl.mem_flags.COPY_HOST_PTR, hostbuf=np.array([self.num_elem]))
        ...
        for t_i in range(len(self.time)):
            ...
            bld.Pos(queue, [self.num_elem], None, pos_x_buf, pos_y_buf, a_x, a_y, res_pos_x, res_pos_y, v_x_buf, v_y_buf,\
            ...
            bld.Speed(queue_speed, [self.num_elem], None, pos_x_buf, pos_y_buf, a_x, a_y, v_x_buf, v_y_buf, res_v_x, res_v_y, \
                        mas_buf, num_elem_buf, dt_buf)
...
#------------------------------------------------------------------
    ...
    def Plot_dif(self):
        ...
        plt.plot(np.sqrt( np.sum((self.Sol_ode - self.Sol_verl_proc)**2, axis=-1) ), 'r', label='Error Verlet with multiprocessing')
