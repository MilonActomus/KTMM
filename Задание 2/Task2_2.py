...
def Verlet_thr(num_elem, Time, delt_t, pos_x, pos_y, v_x, v_y, a, G, mas):
    ...
    for t_i in range(len(Time)):
        ...
        with ThreadPoolExecutor(max_workers = num_elem) as executor:
            jobs = []
            for i in range(num_elem):
                jobs.append(executor.submit(A_th, pos_x = pos_x, pos_y = pos_y, G = G, mas = mas, pl_i = i ))
...
def Verlet_Cyth(num_elem, Time, delt_t, pos_x, pos_y, v_x, v_y, a, G, mas):
    ...
    for t_i in range(len(Time)):
        ...
        a_old = a
        for i in range(num_elem):
            res = A_cyth.A_th(list(pos_x), list(pos_y), G, list(mas), i)
...
def Plot(count, time_Verl, time_Verl_thr, time_Verl_proc, time_Verl_Cyth, time_Verl_CL):
    ...
    plt.plot(count, time_Verl_proc, 'r', label='Verlet with process')
    ...
    plt.plot(count, time_Verl / time_Verl_proc, 'r', label='Verlet with process')
...
def main():
    ...
    for c_i in range(len(count)):
        ...
        time_Verl_proc[c_i] /= 3
        print("Verlet with process ", " c_i= ", c_i, " time: ", time_Verl_proc[c_i])
