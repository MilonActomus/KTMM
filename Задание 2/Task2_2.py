...
def Verlet_thr(num_elem, Time, pos_x, pos_y, v_x, v_y, a, mas):
    ...
    for t_i in range(Time):
        ...
        with ThreadPoolExecutor(max_workers = num_elem) as executor:
            jobs = []
            for i in range(num_elem):
                jobs.append(executor.submit(Pos, pos_x = pos_x, pos_y = pos_y, v_x = v_x, v_y = v_y, a_x = a_x, a_y = a_y, mas = mas, delt_t = delt_t, size = num_elem, i = i ))
        ...
        with ThreadPoolExecutor(max_workers = num_elem) as executor:
            jobs = []
            for i in range(num_elem):
                jobs.append(executor.submit(Speed, pos_x = pos_x, pos_y = pos_y, v_x = v_x, v_y = v_y, a_x = a_x, a_y = a_y, mas = mas, delt_t = delt_t, size = num_elem, i = i ))
...
def Verlet_Cyth(num_elem, Time, pos_x, pos_y, v_x, v_y, a, mas):
    ...
    for t_i in range(Time):
        ...
        pos_x, pos_y, a_x, a_y = Cyth.Pos(pos_x, pos_y, v_x, v_y, a_x, a_y, mas, delt_t, num_elem)

        v_x, v_y, a_x, a_y = Cyth.Speed(pos_x, pos_y, v_x, v_y, a_x, a_y, mas, delt_t, num_elem)
...
def Verlet_CL(num_elem, Time, pos_x, pos_y, v_x, v_y, a, mas, N1, N2):
    ...
    for t_i in range(Time):
        # pos_x_i.append(np.array(pos_x).astype(np.float32))
        # pos_y_i.append(np.array(pos_y).astype(np.float32))
        # speed_x_i.append(np.array(v_x).astype(np.float32))
        # speed_y_i.append(np.array(v_y).astype(np.float32))
        ...
        bld.Pos(queue, [num_elem], None, pos_x_buf, pos_y_buf, a_x, a_y, res_pos_x, res_pos_y, v_x_buf, v_y_buf,\
                    mas_buf, num_elem_buf, dt_buf)
        ...
        bld.Speed(queue_speed, [num_elem], None, pos_x_buf, pos_y_buf, a_x, a_y, v_x_buf, v_y_buf, res_v_x, res_v_y, \
                    mas_buf, num_elem_buf, dt_buf)
...
def Plot(count, time_Verl, time_Verl_thr, time_Verl_proc, time_Verl_proc_pool, time_Verl_Cyth, time_Verl_CL, time_Verl_CL00, time_Verl_CL10, time_Verl_CL11):
    ...
    # # plt.plot(count, time_Verl / time_Verl_CL00, 'r', label='Verlet with OpenCL NVIDIA CUDA')
    # # plt.plot(count, time_Verl / time_Verl_CL10, 'g', label='Verlet with OpenCL Intel(R) HD Graphics 4600')
    # # plt.plot(count, time_Verl / time_Verl_CL11, 'b', label='Verlet with OpenCL Intel(R) Core(TM) i7-4710HQ CPU @ 2.50GHz')
    plt.legend(loc='best')
    plt.xlabel('N')
    plt.grid()
